                ...
                <div class="card-body p-4">
                    <form action="{{ route('client.kyc.submit') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group mb-3">
                            <label class="font-weight-bold text-white small">{{ __('Document / Verification Field') }} <span class="text-danger">*</span></label>
                            <select name="document_type" class="form-control" style="background: #11151e; border: 1px solid #28303f; color: #ffffff;" required>
                                @if(!empty($kycFields))
                                    @foreach($kycFields as $f)
                                        <option value="{{ $f['label'] }}">{{ $f['label'] }} ({{ $f['type'] }})</option>
                                    @endforeach
                                @else
                                    <option value="Full Legal Name">Full Legal Name</option>
                                    <option value="ID Front Scan (Upload)">ID Front Scan (Upload)</option>
                                    <option value="ID Back Scan (Upload)">ID Back Scan (Upload)</option>
                                    <option value="Proof of Address">Proof of Address (Utility Bill / Bank Statement)</option>
                                @endif
                            </select>
