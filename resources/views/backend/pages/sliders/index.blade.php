                ...
                <div class="card-header d-block">
                    <div class="row">
                        ...
                        <div class="col-md-6 col-sm-12 text-right">
                                <a href="{{ route('admin.slider.create') }}" id="addBlogCategoryBtn" class="btn btn-danger btn-sm rounded"><i class="material-icons">add</i></a>
                        </div>
                    </div>
                </div>
                        ...
                        <table id="slider" class="table bapric_table table-striped table-bordered miw-500" cellspacing="0" width="100%">
                            ...
                            @foreach ($sliders as $slider)
                                <tr>
                                    ...
                                    <td>{{$slider->title}}</td>
                                    <td>
                                        <img src="{{ asset('upload/sliders/'.$slider->bg_image) }}" alt="slider image" width="100" height="80">
                                    </td>
                                    <td>{{ $slider->button_name }}</td>
                                    <td>{{ $slider->button_url }}</td>
                                    <td>
                                        <label class="switch">
                                            <input type="checkbox" {{ $slider->status?'checked':'' }} id="{{ $slider->id }}" class="sliderActivationBtn">
...
@section('page-script')
    @include('backend.pages.sliders.internal-assets.js.delete-warning')
    <script src="{{ asset('backend/assets/js/slider-activation.js') }}"></script>
@endsection
