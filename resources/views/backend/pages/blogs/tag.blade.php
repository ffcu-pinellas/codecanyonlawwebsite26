                                    ...
                                    <td>
                                        <div class="d-flex">
                                            <a href="javascript:void(0)">
                                                <button type="button" class="btn btn-sm btn btn-success m-1 blogCategoryEditBtn" data-id="{{ $tag->id }}">{{ __('Edit') }}</button>
                                            </a>
                                            <a href="javascript:void(0)" title="{{__('Delete')}}" class="deleteRow">
                                                ...
                                                <form action="{{ route('admin.blog.tag.destroy', $tag->id) }}" method="post" class="deleteForm">
                                                    @csrf
                                                    @method('delete')
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title card-title" id="blogCategoryModalLongTitle">{{ __('Add new tag') }}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="{{ route('admin.blog.tag.store') }}" method="post" id="blogCategoryForm">
                    @csrf
